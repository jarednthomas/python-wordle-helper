...
def break_wordle():
    ...
    unvalidated_string = args.input_string
    for char in unvalidated_string:
        ...
        if char in string.ascii_lowercase or char in string.ascii_uppercase or char == "#":
            ...
            sys.exit("Invalid input: [{}]\nOnly letters and # for unknowns are allowed, eg: h#ll#".format(char))

    # Generate valid string from validated
    validated_string = "".join([i for i in validated])

    # Reject valid strings of invalid lengths, double check
    if len(validated_string) != 5:
        sys.exit("Invalid length: {}\nExpected length of 5".format(len(validated_string)))
    ...
    if(wordle_regex): 
        ...
        sys.exit("An error occured while trying to generate regex. Create an issue on github and post the --verbose output, please.\n{}".format(wordle_regex))

    possible_words = []
    # Iterate through dictionary of five letter words with regex
    with open("./five-letter-words/sgb-words.txt") as f:
        for line in f:
